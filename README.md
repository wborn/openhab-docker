...
[![Docker Version](https://img.shields.io/badge/version-5.2.0-blue)](https://hub.docker.com/repository/docker/openhab/openhab/tags?name=5.2.0)
...
* **Stable:** Thoroughly tested semi-annual official releases of openHAB. Use the stable version for your production environment if you do not need the latest enhancements and prefer a robust system.
  * `5.2.0` ([Release notes](https://github.com/openhab/openhab-distro/releases/tag/5.2.0))
...
* `alpine` for Alpine 3.24 ([Dockerfile](https://github.com/openhab/openhab-docker/blob/main/alpine/Dockerfile))
...
If you are unsure about what your needs are, you probably want to use `openhab/openhab:5.2.0`.
...
`docker run --name openhab --net=host openhab/openhab:5.2.0`
...
`docker run --detach --name openhab openhab/openhab:5.2.0`
...
`docker run -it openhab/openhab:5.2.0`

#### Debug mode

The debug mode is started with the command:

`docker run -it openhab/openhab:5.2.0 ./start_debug.sh`
...
docker build \
  ...
  --build-arg OPENHAB_VERSION=5.2.0 \
  --tag openhab/openhab \
  .
```
